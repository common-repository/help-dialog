...
class EPHD_Search_Query {
	...
	private function search_articles( $return_total, $results_from=0, $initial_limit=0 ) {
		...
		$sql_escaped = $this->generate_article_search_query( $return_total, $results_from, $initial_limit );
		$this->audit( 'sql (articles)', $sql_escaped );

		if ( $return_total ) {
			$search_result = $wpdb->get_var( $sql_escaped );
		} else {
			$search_result = $wpdb->get_results( $sql_escaped );
		}
	...
	private function where_like_keywords() {
		...
		foreach ( $this->search_keywords as $search_keyword ) {
			...
			$search_sql_escaped .= $wpdb->prepare( " ( (wp.post_title LIKE %s) ", $search_like );
			...
			if ( in_array( $search_keyword, EPHD_Search_Query_Extras::html_css_keywords() ) ) {
				...
				$search_sql_escaped .= $wpdb->prepare( " OR (wp.post_excerpt LIKE %s) OR (wp.post_content LIKE %s) ", $search_regex, $search_regex );

			} else {

				$search_sql_escaped .= $wpdb->prepare( " OR (wp.post_excerpt LIKE %s) OR (wp.post_content LIKE %s) ", $search_like, $search_like );
			}

			// for the Link Editor search article metadata for search terms as well
			if ( EPHD_Utilities::is_link_editor_enabled() ) {
				$search_sql_escaped .= $wpdb->prepare( " OR ( wpp.meta_key = 'kb"."lk_search_terms' AND wpp.meta_value LIKE %s ) ", $search_like );
			}
	...
	private function search_tags( $results_from=0, $limit=0 ) {
		...
		// check if categories is selected, search only in selected categories
		if ( empty( $this->user_category_ids ) ) {
			$where_escaped = $wpdb->prepare( "WHERE taxonomy = %s ", $kb_tag_taxonomy ) . ( $this->is_amag ? '' : 'AND count > 0' ) . " ) terms ON terms.term_id = t.term_id AND name = %s ";
		} else {
			$where_escaped = $wpdb->prepare( "WHERE (taxonomy = %s OR taxonomy = %s) ", $kb_tag_taxonomy, $kb_category_taxonomy ) . ( $this->is_amag ? '' : 'AND count > 0' ) .
							$wpdb->prepare( " ) terms ON (terms.term_id = t.term_id AND t.term_id in (%s) ", implode( ',', $this->user_category_ids ) ) . " AND name = %s ) ";
		}
		...
		// - in SQL for logged-in users select only those articles which they are allowed to see
			...
			$where_post_status_escaped .= current_user_can( 'read_private_posts' )
				...
				:  $wpdb->prepare( " OR ( p.post_status = 'private' AND p.post_author = %s ) ", get_current_user_id() );
		}

		$where_escaped .= " AND ( {$where_post_status_escaped} ) ";

		$search_result = array();
		foreach ( $tags_search_keywords as $search_keyword ) {

			$sql_escaped = $wpdb->prepare( "SELECT " . ' * ' . "
				FROM {$wpdb->prefix}posts p
					INNER JOIN {$wpdb->prefix}term_relationships tr ON p.ID = tr.object_id
					INNER JOIN {$wpdb->prefix}terms t ON t.term_id = tr.term_taxonomy_id
					INNER JOIN (SELECT term_id
								FROM {$wpdb->prefix}term_taxonomy {$where_escaped} LIMIT %d, %d", $search_keyword, $results_from, $limit );

			$this->audit( 'sql (tags)', $sql_escaped );

			$result = $wpdb->get_results( $sql_escaped );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
