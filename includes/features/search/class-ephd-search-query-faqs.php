...
class EPHD_Search_Query_FAQs {
	...
	private function search_articles( $return_total, $results_from=0, $initial_limit=0 ) {
		...
		if ( $return_total ) {
			$search_result = $wpdb->get_var( $sql_escaped );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		} else {
			$search_result = $wpdb->get_results( $sql_escaped );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	private function where_like_keywords() {
		...
		foreach ( $this->search_keywords as $search_keyword ) {
			...
			$like_clause_escaped = " ( (wp.question LIKE %s) ";
			...
			if ( in_array( $search_keyword, EPHD_Search_Query_Extras::html_css_keywords() ) ) {
				...
				$like_clause_escaped .= " OR (wp.answer LIKE %s) ";
				$search_like_args[] = $search_regex;
			...
			$search_sql .= $wpdb->prepare( " {$and_escaped} " . $like_clause_escaped, $search_like_args );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	private function assemble_order_by() {
		...
		foreach ( $this->search_keywords as $search_keyword ) {
			$search_like = '%' . $wpdb->esc_like( $search_keyword ) . '%';
			// TODO $search_like = '([^[:alnum:]|-|_|@]|^)' . $wpdb->esc_like( $search_keyword ) . '';
			$order_by_search_title[] = $wpdb->prepare( "wp.question LIKE %s", $search_like );
		}
		...
		$search_order_by = $wpdb->prepare( "WHEN wp.question LIKE %s THEN 1 ", $like );
		...
		$search_order_by .= $wpdb->prepare( "WHEN wp.answer LIKE %s THEN 4 ", $like );
