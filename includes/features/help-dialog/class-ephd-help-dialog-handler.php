...
class EPHD_Help_Dialog_Handler {
	...
	public static function add_default_faqs() {
		...
		foreach ( $demo_questions as $question ) {
			...
			$widgets_config[ EPHD_Config_Specs::DEFAULT_ID ]['faqs_sequence'][] = $question->faq_id;
		}
	...
	private static function get_demo_questions() {
		...
		$faqs_config = [
			[
				'question' => esc_html__( 'Where can I find documentation?', 'help-dialog' ),
			...
			[
				'question' => esc_html__( 'Do you offer any discounts?', 'help-dialog' ),
				'answer' => esc_html__( 'EXAMPLE', 'help-dialog' ) . ' - ' . esc_html__( 'Currently, we have a sale for 20% off all regular priced merchandise in the store.', 'help-dialog' ),
			],
			[
				'question' => esc_html__( 'What payment methods do you accept?', 'help-dialog' ),
				'answer' => esc_html__( 'EXAMPLE', 'help-dialog' ) . ' - '. esc_html__( 'We accept all main methods of payments: VISA, Mastercard, and PayPal.', 'help-dialog' ),
			],
		];
