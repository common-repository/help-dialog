...
class EPHD_Utilities {
	...
	public static function ajax_verify_nonce_and_admin_permission_or_error_die() {
		...
		if ( empty( $wp_nonce ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $wp_nonce ) ), '_wpnonce_ephd_ajax_action' ) ) {
			self::ajax_show_error_die( esc_html__( 'Login or refresh this page to edit this.', 'help-dialog' ) . ' (E01)' );
		}

		// ensure user has correct permission
		if ( ! current_user_can( self::ADMIN_CAPABILITY ) ) {
			self::ajax_show_error_die( esc_html__( 'Login or refresh this page', 'help-dialog' ) . ' (E02)' );
		}
	}
	 ...
	 */
	public static function ajax_verify_nonce_and_capability_or_error_die( $capability ) {

		// check wpnonce
		$wp_nonce = self::post( '_wpnonce_ephd_ajax_action' );
		if ( empty( $wp_nonce ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $wp_nonce ) ), '_wpnonce_ephd_ajax_action' ) ) {
			self::ajax_show_error_die( esc_html__( 'Login or refresh this page to edit this.', 'help-dialog' ) . ' (E03)'  );
		}

		// no need to check permission if capability false or user admin
		...
		if ( ! current_user_can( $capability ) ) {
			self::ajax_show_error_die( esc_html__( 'You do not have permission to edit this Help Dialog.', 'help-dialog' ) . ' (E04)' );
		}
	}
	...
	public static function ajax_verify_nonce_and_prevent_direct_access_or_error_die() {
		...
		if ( empty( $wpnonce_value ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $wpnonce_value ) ), '_wpnonce_ephd_ajax_action' ) ) {
			self::ajax_show_error_die( esc_html__( 'Please refresh your page.', 'help-dialog' )  . ' (E03)');
		}

		// prevent direct access ?
		if ( ! did_action( 'init' ) && ! did_action( 'admin_init' ) ) {
			self::ajax_show_error_die( esc_html__( 'Error occurred', 'help-dialog' ) . ' (E04)' );
		}
	}
	...
	public static function user_not_logged_in() {
		if ( defined('DOING_AJAX') ) {
			self::ajax_show_error_die( '<p>' . esc_html__( 'You are not logged in. Refresh your page and log in.', 'help-dialog' ) . '</p>', esc_html__( 'Cannot save your changes', 'help-dialog' ) );
		}
	}
	...
	public static function get_wp_option( $option_name, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null ) {
			return new WP_Error( '432', esc_html__( 'Error occurred', 'help-dialog' ) . ' ' . $option_name );
		}
	...
	public static function save_wp_option( $option_name, $option_value ) {
		...
		if ( is_array( $option_value ) || is_object( $option_value ) ) {
			...
			if ( empty( $serialized_value ) ) {
				return new WP_Error( '434', esc_html__( 'Error occurred', 'help-dialog' ) . ' ' . $option_name );
			}
	...
	public static function get_postmeta( $post_id, $meta_key, $default, $is_array=false, $return_error=false ) {
		...
		if ( ! self::is_positive_int( $post_id) ) {
			return $return_error ? new WP_Error( esc_html__( 'Error occurred', 'help-dialog' ), self::get_variable_string( $post_id ) ) : $default;
		}
	...
	public static function save_postmeta( $post_id, $meta_key, $meta_value, $sanitized ) {
		...
		if ( ! self::is_positive_int( $post_id) ) {
			return new WP_Error( esc_html__( 'Error occurred', 'help-dialog' ), self::get_variable_string( $post_id ) );
		}

		if ( $sanitized !== true ) {
			return new WP_Error( '433', esc_html__( 'Error occurred', 'help-dialog' ) . ' ' . $meta_key );
		}
		...
		if ( is_array( $meta_value ) || is_object( $meta_value ) ) {
			...
			if ( empty($serialized_value) ) {
				return new WP_Error( '434', esc_html__( 'Error occurred', 'help-dialog' ) . ' ' . $meta_key );
			}
		}
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			...
			return new WP_Error(esc_html__( 'Error occurred', 'help-dialog' ), $wpdb_last_error);
		}
		...
		if ( empty($result) ) {
			if ( false === $wpdb->query( $wpdb->prepare( "INSERT INTO $wpdb->postmeta (`meta_key`, `meta_value`, `post_id`) VALUES (%s, %s, %d)", $meta_key, $serialized_value, $post_id ) ) ) {
				...
				return new WP_Error( '33', esc_html__( 'Error occurred', 'help-dialog' ) );
			}
		} else {
		...
		if ( $result === false ) {
			...
			return new WP_Error( '435', esc_html__( 'Error occurred', 'help-dialog' ) . ' ' . $meta_key );
		}
	...
	public static function send_email( $message, $is_message_html, $to_support_email='', $reply_to_email='', $reply_to_name='', $subject='' ) {
		...
		if ( empty( $message ) || strlen( $message ) > 5000 ) {
			...
			return esc_html__( 'Error occurred', 'help-dialog' ) . ' (0)';
		}
		...
		if ( empty( $to_support_email ) || strlen( $to_support_email) > 100 ) {
			return esc_html__( 'Invalid email', 'help-dialog' ) . ' (1)';
		}

		if ( ! is_email( $to_support_email ) ) {
			return esc_html__( 'Invalid email', 'help-dialog' ) . ' (2)';
		}
		...
		if ( empty( $reply_to_email ) || strlen( $reply_to_email ) > 100 ) {
			return esc_html__( 'Invalid email', 'help-dialog' ) . ' (3)';
		}
		
		if ( ! is_email( $reply_to_email ) ) {
			return esc_html__( 'Invalid email', 'help-dialog' ) . ' (4)';
		}
		
		if ( strlen( $reply_to_name ) > 100 ) {
			return esc_html__( 'Error occurred', 'help-dialog' ) . ' (5)';
		}
		...
		if ( strlen( $subject ) > 200 ) {
			return esc_html__( 'Invalid subject', 'help-dialog' );
		}

		if ( strlen( $message ) > 10000 ) {
			return esc_html__( 'Email message is too long', 'help-dialog' );
		}
		...
		$error_message = esc_html__( 'Failed to send the email.', 'help-dialog' );
	...
	/**
	 * Return true if the AI Chat is available, false otherwise
	 * @return false|mixed|null
	 */
	public static function is_hd_pro_chat_available() {
		return has_filter( 'ephd_help_dialog_is_chat_available' ) ? apply_filters( 'ephd_help_dialog_is_chat_available', false ) : false;
	}
}
