...
class EPHD_Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		if ( empty( $input ) ) {
			return new WP_Error( 'invalid_input', esc_html__( 'Error Occurred', 'help-dialog' ) . ' (5532)' );
		}
	...
	private function filter_input_field( $value, $field_spec ) {
		...
		switch ( $field_spec['type'] ) {
			...
			default:
				return new WP_Error('ephd-invalid-input-type', esc_html__( 'Error Occurred', 'help-dialog' ) . ' - ' . $field_spec['type']);
		}
	}
	...
	private function filter_select( $value, $field_spec ) {
		...
		if ( ! in_array( $value, array_keys($field_spec['options']) )  && ! empty($field_spec['mandatory']) ) {
			$value_text = ( empty($value) ? esc_html__( 'empty', 'help-dialog' ) : '"' . $value . '"' );
			/* translators: 1: value entered by user, 2: HTML tag, 3: valid values user should enter. */
			$msg = '<br>' . sprintf( esc_html__( 'The value cannot be %1$s. %2$sValid values are: %3$s', 'help-dialog' ), $value_text, '<br>', implode(", ", $field_spec['options']) );
	...
	private function filter_checkbox( $value ) {
		...
		return new WP_Error('filter_checkbox_invalid', sprintf( esc_html__( 'The value "%s" is not valid', 'help-dialog' ), $value ) );
	}
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		if ( $number != $number_int ) {
			/* translators: %s: value entered by user. */
			return new WP_Error('filter_not_number',sprintf( esc_html__( 'The value "%s" is not a number', 'help-dialog' ), $number ) );
		}

		if ( $number > $field_spec['max'] ) {
			/* translators: 1: value entered by user, 2: maximum length user should enter. */
			$msg = sprintf( esc_html__( 'The value %1$s is larger than maximum of %2$s', 'help-dialog' ), $number, $field_spec['max'] );
			...
			$msg = sprintf( esc_html__( 'The value %1$s is smaller than minimum of %2$s', 'help-dialog' ), $number, $field_spec['min'] );
			return new WP_Error( 'filter_not_number', $msg );
		}
	...
	private function filter_true_false( $boolean ) {
		...
		return new WP_Error( 'filter_not_number', esc_html__( 'Error Occurred', 'help-dialog' ) . ' (112)' );
	}
	...
	public function filter_color_hex( $value, $field_spec=array() ) {
		...
		/* translators: %s: value entered by user. */
		return new WP_Error('filter_not_color_hex', sprintf( esc_html__( 'The value "%s" is not a valid HEX color. Expected format: #xxxxxx', 'help-dialog' ), $value ));
	}
	...
	private function filter_id( $id ) {
		...
		if ( is_wp_error($id) ) {
			return new WP_Error('filter_not_id', esc_html__( 'Error Occurred', 'help-dialog' ) . ' - ' . $id . ' - ' . $id->get_error_code() );
		}
		return $id;
	}
	...
	private function filter_enumeration( $value, $field_spec ) {
		...
		return new WP_Error('filter_not_enumeration', esc_html__( 'Error Occurred', 'help-dialog' ) . ' - ' . $value );
	}
	...
	private function filter_typography( $value ) {
		...
		if ( ! is_array( $value ) ) {
			return new WP_Error( 'filter_not_typograghy', esc_html__( 'Error Occurred', 'help-dialog' ) . ' - ' . EPHD_Utilities::get_variable_string( $value ) );
		}
