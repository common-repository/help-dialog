...
class EPHD_Admin_UI_Access {
	...
	public static function save_access_control( $global_config ) {
		...
		// we are done here
		// EPHD_Utilities::ajax_show_info_die( esc_html__( 'Configuration saved', 'help-dialog' ) );
        return $global_config;
	}
	private static function get_admins_distinct_box() {
		return sprintf( esc_html__( '%sAdmins%s', 'help-dialog' ), '<span class="ephd-admin__distinct-box ephd-admin__distinct-box--high">', '</span>' );
	}

	private static function get_editors_distinct_box() {
		return sprintf( esc_html__( '%sEditors%s', 'help-dialog' ), '<span class="ephd-admin__distinct-box ephd-admin__distinct-box--middle">', '</span>' );
	}

	private static function get_authors_distinct_box() {
		return sprintf( esc_html__( '%sAuthors%s', 'help-dialog' ), '<span class="ephd-admin__distinct-box ephd-admin__distinct-box--low">', '</span>' );
	}

	private static function get_users_with_capability_distinct_box( $capability ) {
		return sprintf( esc_html__( '%susers with "%s" capability%s', 'help-dialog' ), '<span class="ephd-admin__distinct-box ephd-admin__distinct-box--lowest">', $capability, '</span>' );
	}
