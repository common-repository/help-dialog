...
class EPHD_Delete_HD {
	...
	public function get_delete_all_help_dialog_data_form() {
		...
		foreach ( $this->message as $class => $message ) {
			echo wp_kses_post( EPHD_HTML_Forms::notification_box_bottom( $message, '', $class ) );
		}
	...
	private function handle_form_actions() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_ephd_ajax_action'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce_ephd_ajax_action'] ) ), '_wpnonce_ephd_ajax_action' ) ) {
			$this->message['error'] = esc_html__( 'Error occurred', 'help-dialog' ) . ' (1)';
			return;
		}

		// ensure user has correct permissions
		if ( ! current_user_can( 'manage_options' ) ) {
			$this->message['error'] = esc_html__( 'You do not have permission.', 'help-dialog' );
			return;
		}
	...
	private function reset_all_handler( $action ) {
		...
		if ( EPHD_Utilities::post( 'reset_text' ) != 'reset' ) {
			$this->message['error'] = sprintf( esc_html__( 'Write "%s" in input box to delete ALL Help Dialog configuration', 'help-dialog' ), 'reset' );
			return;
		}

		self::reset_config();

		$this->message['success'] = esc_html__( 'All configs were reset.', 'help-dialog' );
	}
	...
	private function delete_all_handler( $action ) {
		...
		if ( EPHD_Utilities::post( 'delete_text' ) != 'delete' ) {
			$this->message['error'] = sprintf( esc_html__( 'Write "%s" in input box to delete ALL Help Dialog configuration', 'help-dialog' ), 'delete' );
			return;
		}
		...
		$this->message['success'] = esc_html__( 'All data will be deleted upon plugin uninstallation.', 'help-dialog' );
	}
	...
	public static function get_reset_config_button() {
        ...
        //Call handler for reset button
        EPHD_Delete_HD::reset_config_button_handler( true );
     ...
     * @param bool $redirect - Redirect to admin page advanced tab if true
	 */
	public static function reset_config_button_handler( $redirect = false ) {
        ...
        if ( $redirect ) { ?>
            <script type="text/javascript">
                window.location.href="<?php echo esc_url( admin_url( '/admin.php?page=ephd-help-dialog-advanced-config' ) ); ?>";
            </script>
            <?php exit;
        }
	}
