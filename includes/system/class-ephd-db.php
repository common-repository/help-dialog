...
abstract class EPHD_DB {
	...
	public function get_a_row_by_column_value( $column_name, $column_value ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			$wpdb_last_error = $wpdb->last_error;   // add_log changes last_error so store it first
			EPHD_Logging::add_log( "DB failure: " . $wpdb_last_error, 'Column - value: ' . $column_name . ' - ' . $column_value );
			return new WP_Error('DB failure', $wpdb_last_error);
		}
	...
	public function clear_table() {
		global $wpdb;
		return $wpdb->query( "DELETE FROM {$this->table_name}" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
	...
	public function delete_table() {
		global $wpdb;
		return $wpdb->query( "DROP TABLE IF EXISTS {$this->table_name}" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
