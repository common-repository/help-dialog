...
class EPHD_KB_Core_Utilities {
	...
	public static function get_kb_configs() {
		...
		$kb_configs = $wpdb->get_results( $wpdb->prepare( "SELECT option_value FROM $wpdb->options WHERE option_name LIKE %s",
															EPHD_KB_Core_Utilities::KB_CONFIG_PREFIX . '%' ), ARRAY_A );
		...
		foreach ( $kb_configs as $ix => $row ) {
			...
			if ( empty( $config ) || ! is_array( $config ) ) {
				EPHD_Logging::add_log( "Did not find configuration" );
				return null;
			}
