...
class EPHD_Widgets_DB extends EPHD_DB {
	...
	public function update_config( $widgets_config ) {
		...
		// ensure default widget is always present in the widgets configuration
		if ( ! key_exists( self::DEFAULT_ID, $widgets_config ) ) {
			$widgets_config = [ self::DEFAULT_ID => $this->default_config ] + $widgets_config;
		}
	...
	public function get_config( $return_error=false ) {
		...
		if ( is_wp_error( $widget_config_records ) ) {
			EPHD_Logging::add_log( "Can't get Widget records", $widget_config_records );
			if ( $return_error ) {
				return new WP_Error( 'DB231', esc_html__( "Did not find Help Dialog configuration", 'help-dialog' ) . ': ' . $widget_config_records->get_error_message() . '' );
			}
		...
		if ( empty( $widget_config_records ) || ! is_array( $widget_config_records ) ) {
			EPHD_Logging::add_log( "Can't get Widget records, trying to re-create widgets configuration with the default widget." );

			// try to re-create default widget on empty table
			$re_created_widgets_config = $this->update_config( [ self::DEFAULT_ID => $this->default_config ] );
			if ( is_wp_error( $re_created_widgets_config ) ) {
				EPHD_Logging::add_log( "Failed to re-create default widgets configuration." );
				if ( $return_error ) {
					return new WP_Error( 'DB231', esc_html__( "Did not find Help Dialog configuration.", 'help-dialog' ) . ' (E77) ' );
				}
			}
		...
		foreach ( $widget_config_records as $widget_config_obj ) {
			...
			if ( empty( $widget_config_obj ) ) {
				...
				if ( $return_error ) {
					return new WP_Error( 'DB231', esc_html__( "Did not find Help Dialog configuration.", 'help-dialog' ) . ' (E78) ' );
				}
	...
	public function reset_config() {
		...
		if ( ! parent::installed() ) {
			EPHD_Logging::add_log( 'Cannot find widgets table on config reset. Trying to re-create the table.' );

			// try to re-create the table if not exists
			$this->create_table();

			// report error if still cannot create the table
			if ( ! parent::installed() ) {
				EPHD_Logging::add_log( 'Failed to re-create widgets table on config reset.' );
				return new WP_Error( 'DB231', esc_html__( "Cannot find Widgets table.", 'help-dialog' ) . ' (E75) ' );
			}
		}
		...
		if ( empty( $result ) ) {
			...
			return new WP_Error( 'DB231', esc_html__( "Cannot clear Widgets table.", 'help-dialog' ) . ' (E79) ' );
		}

		// populate the table with default widget
