...
class EPHD_Admin_Ctrl {
	...
	public function save_global_settings() {
		...
		$global_config['contact_submission_email'] = EPHD_Utilities::post( 'contact_submission_email', $global_config['contact_submission_email'], $global_config_specs['contact_submission_email']['type'],
																			intval( $global_config_specs['contact_submission_email']['max'] ) );
		if ( ! empty( $_POST['contact_submission_email'] ) && ! is_email( $global_config['contact_submission_email'] ) ) {
			EPHD_Utilities::ajax_show_error_die( esc_html__( 'Please enter a valid email address.', 'help-dialog' ) );
		}
		...
		// only update if user entered a new key
		$new_open_api_key = EPHD_Utilities::post( 'ephd_openai_key' );
		if ( empty( $new_open_api_key ) || strpos( $new_open_api_key, '...' ) === false ) {
			$result = EPHD_OpenAI::save_openai_api_key( $new_open_api_key );
			if ( is_wp_error( $result ) ) {
				EPHD_Logging::add_log( 'Error occurred on saving Global configuration. (37)' );
				EPHD_Utilities::ajax_show_error_die( EPHD_Utilities::report_generic_error( 37, $result ) );
			}
		}
	...
	public static function handle_form_actions() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_manage_hd'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce_manage_hd'] ) ), '_wpnonce_manage_hd' ) ) {
			return [ 'error' => EPHD_Utilities::report_generic_error( 1 ) ];
		}

		// only admin user can handle these actions
		if ( ! current_user_can( 'manage_options' ) ) {
			return [ 'error' => esc_html__( 'You do not have permission.', 'help-dialog' ) ];
		}
