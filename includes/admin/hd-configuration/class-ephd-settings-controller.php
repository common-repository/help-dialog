...
class EPHD_Settings_Controller {
	 ...
	 * @return string
	 */
	private static function display_debug_data() {
		/** @var $wpdb Wpdb */
		global $wpdb;

		// ensure user has correct permissions
		if ( ! current_user_can( 'manage_options' ) ) {
			return esc_html__( 'You do not have permission to edit Help Dialog.', 'help-dialog' );
		}

		$first_plugin_version = ephd_get_instance()->global_config_obj->get_value( 'first_plugin_version' );
		...
		$output .= "==================\n";
		$output .= "HD first version: " . $first_plugin_version . "\n";
	...
	public static function display_debug_info() {
		...
		<div id="ephd_debug_info_tab_page">     <?php
			...
			EPHD_HTML_Elements::submit_button_v2(
				$is_debug_on ? esc_html__( 'Disable Debug', 'help-dialog' ) : esc_html__( 'Enable Debug', 'help-dialog' ),
			...
			if ( $is_debug_on ) {       ?>
				...
				echo self::display_debug_data(); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped        ?>

				<form action="<?php echo esc_url( admin_url( 'admin.php?page=ephd-help-dialog-advanced-config#debug' ) ); ?>" method="post" dir="ltr">   <?php

					EPHD_HTML_Elements::submit_button_v2(
							esc_html__( 'Download System Information', 'help-dialog' ),
