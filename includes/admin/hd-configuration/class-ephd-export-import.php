...
class EPHD_Export_Import {
	...
	public function import_hd_config() {
		...
		if ( pathinfo( $sanitized_file_name, PATHINFO_EXTENSION ) !== 'json' ) {
			$this->message['error'] = esc_html__( 'Import file format is not correct.', 'help-dialog' ) . ' (5)' . esc_html__( 'File', 'help-dialog' ) . ': ' . esc_html( $sanitized_file_name );
			return $this->message;
		}
		...
		if ( empty( $import_data_file ) ) {
			$this->message['error'] = esc_html__( 'Import file format is not correct.', 'help-dialog' ) . ' (7)';
			return $this->message;
		}

		// validate the file is JSON and imported data is an array
		$imported_configs = json_decode( $import_data_file, true );
		if ( json_last_error() !== JSON_ERROR_NONE || empty( $imported_configs ) || ! is_array( $imported_configs ) ) {
			$this->message['error'] = esc_html__( 'Import file format is not correct.', 'help-dialog' ) . ' ' . json_last_error_msg() . ' (8)';
			return $this->message;
		}
		...
		if ( self::is_imported_version_too_new( $imported_configs ) ) {
			$this->message['error'] = esc_html__( 'The importing configuration version is too new for this plugin. Please update the plugin first.', 'help-dialog' ) . ' (9)';
			return $this->message;
		}
	...
	private function update_all_hd_configs( $new_configs ) {
		...
		$this->message['success'] =  esc_html__( 'Import finished successfully', 'help-dialog' );
	}
	...
	private function upgrade_imported_data( &$imported_configs ) {
		...
		if ( version_compare( $import_plugin_version, Echo_Help_Dialog::$version, '<' ) ) {
			...
			// run upgrades
			// TODO EPHD_Upgrades::run_upgrades( $imported_global_config, $imported_widgets_config, $imported_designs_config, $imported_contact_forms_config, $import_plugin_version );
