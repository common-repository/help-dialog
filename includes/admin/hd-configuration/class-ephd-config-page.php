...
class EPHD_Config_Page {
	...
	public function display_page() {
		...
		if ( ! current_user_can( EPHD_Admin_UI_Access::get_context_required_capability( ['admin_ephd_access_admin_pages_read'] ) ) ) {
			EPHD_Utilities::ajax_show_error_die( esc_html__( 'You do not have permission to edit Help Dialog.', 'help-dialog' ) );
			return;
		}

		$global_config = ephd_get_instance()->global_config_obj->get_config( true );
		if ( is_wp_error( $global_config ) ) {
			$error_message = esc_html__( 'Cannot load configuration.', 'help-dialog' );
			$error_html    = EPHD_Utilities::is_user_admin()
			...
			EPHD_HTML_Admin::display_config_error_page( $global_config, $error_message, $error_html );
			return;
		}
	...
	private static function get_hidden_help_dialog_element_classes_form( $global_config ) {
		...
		EPHD_HTML_Elements::textarea( array(
			...
			'input_size' => 'large',
			'rows'       => 3,
			'desc'       => esc_html__( 'Add a comma separated list of CSS classes', 'help-dialog' ),
		) );
	...
	private static function show_license_boxes( $license_content ) {
		...
		return array(
			...
			// Box: Licenses
			array(
				'title' => esc_html__( 'License for Help Dialog PRO', 'help-dialog' ),
				'description' => self::get_licenses_box_description(),
				'html' => wp_kses_post( $license_content ),
			)
		);
	}
	...
	private static function get_licenses_box_description() {
		return sprintf( esc_html__( 'You can access your license account %s here%s' , 'help-dialog' ), '<a href="https://www.helpdialog.com/account-dashboard/" target="_blank" rel="noopener">', '</a>' ) .
			'<br />' . sprintf( esc_html__( 'Please refer to the %s documentation%s for help with your license account and any other issues.', 'help-dialog' ), '<a href="https://www.helpdialog.com/documentation/license-faqs/" target="_blank" rel="noopener">', '</a>');
	}
	...
	private static function settings_tab_included_user_roles_box( $included_roles ) {
		...
		EPHD_HTML_Elements::checkboxes_multi_select( array(
				'label'             => esc_html__( 'Include specific user roles from Private FAQs access', 'help-dialog' ),
	...
	private function get_regular_views_config( $global_config, $global_specs ) {
		...
		$regular_views[] = array(
			...
			'boxes_list' => array(
				...
				array(
					'title'         => esc_html__( 'OpenAI', 'help-dialog' ),
					'html'          => self::get_openai_form( $global_config ),
				),
				// Box: WPML
				array(
					'title'         => esc_html__( 'Polylang/WPML Setup', 'help-dialog' ),
					'html'          => self::get_polylang_wpml_form( $global_config ),
				),
				// Box: Hide KB article elements
				array(
					'title'         => esc_html__( 'Hide Some Content of KB Articles', 'help-dialog' ),
				...
				// Box: reset config
				array(
					'title' => esc_html__( 'Reset HD config', 'help-dialog' ),
					'html'  => self::get_reset_settings_html(),
				),
				// Box: Delete All Help Dialog Data
				array(
					'title' => esc_html__( 'Help Dialog Plugin Removal', 'help-dialog' ),
		...
		$regular_views[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Debug', 'help-dialog' ),
			...
			'boxes_list' => array(
				...
				array(
					'title' => esc_html__( 'Information required for support', 'help-dialog' ),
					'description' => esc_html__( 'Enable debugging when instructed by the Echo team.', 'help-dialog' ),
		...
		if ( ! empty( $license_content ) ) {
			$regular_views[] = [
				...
				// Top Panel Item
				'label_text' => esc_html__( 'Licenses', 'help-dialog' ),
	...
	private static function get_polylang_wpml_form( $global_config ) {
		...
		// show ad box if the HD PRO is not enabled
		if ( ! EPHD_Utilities::is_help_dialog_pro_enabled() ) {
			return EPHD_HTML_Forms::pro_feature_ad_box( array(
				'id' => 'ephd-dialog-pro-feature-ad-initial-message',
				'class' => 'ephd-admin-pro-feature-ad-container--wide',
				'title' => sprintf( __( "Add %sTranslation Text with Polylang and WPML%s ", 'help-dialog' ), '<strong>', '</strong><br />' ),
				'list' => array(
				),
				'btn_text' => esc_html__( 'Upgrade Now', 'help-dialog' ),
				'btn_url' => 'https://www.helpdialog.com/documentation/translate-text-with-polylang-and-wpml/',
				'show_close_btn' => 'yes',
				'return_html' => true,
			) );
		}
	...
	private static function get_openai_form( $global_config ) {
		...
		$api_key = EPHD_OpenAI::get_openai_api_key();
		...
		'input_size'    => 'small',
		'tooltip_body'  => esc_html__( 'The maximum number of tokens to generate in the completion. The token count of your prompt plus max_tokens cannot exceed the model\'s context length. Most models have a context length ' .
		...
		target="_blank" rel="noopener">' . esc_html__( 'See it in OpenAI Documentation', 'help-dialog' ) . '</a>',
		] ); */
