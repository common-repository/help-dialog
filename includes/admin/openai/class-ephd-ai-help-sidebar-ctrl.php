...
class EPHD_AI_Help_Sidebar_Ctrl {
	...
	public function fix_question_spelling_and_grammar() {
		...
		$input_text = 'Fix spelling and grammar of the following text:'.$input_text;

		$openai_handler = new EPHD_OpenAI();
		$fixed_input_text = $openai_handler->complete(
			self::DEFAULT_MODEL,
			$input_text
		);

		// let user see error message from OpenAI API response
		if ( is_wp_error( $fixed_input_text ) ) {
			EPHD_Utilities::ajax_show_error_die( $fixed_input_text->get_error_message() );
		}

		wp_die( wp_json_encode( array(
			'status'            => 'success',
			'message'           => esc_html__( 'Spelling and grammar have been successfully corrected.', 'help-dialog' ),
	...
	public function create_five_question_alternatives() {
		...
		$completion_text = $openai_handler->complete(
			self::DEFAULT_MODEL,
			'Create a numbered list of five alternative questions of up to 200 characters each to this text: ' . $input_text,
			0.5
		);

		// let user see error message from OpenAI API response
		if ( is_wp_error( $completion_text ) ) {
			EPHD_Utilities::ajax_show_error_die( $completion_text->get_error_message() );
		}
		...
		wp_die( wp_json_encode( array(
			'status'        => 'success',
			'message'       => esc_html__( 'Successfully generated.', 'help-dialog' ),
	...
	public function create_five_answer_alternatives() {
		...
		$completion_text = $openai_handler->complete(
			self::DEFAULT_MODEL,
			'Create a numbered list of five alternatives of up to 1500 characters each to this text: ' . $input_text,
			0.5
		);
	...
	public function create_answer_based_on_question() {
		...
		// let user see error message from OpenAI API response
		if ( is_wp_error( $generated_answer ) ) {
			EPHD_Utilities::ajax_show_error_die( $generated_answer->get_error_message() );
		}
