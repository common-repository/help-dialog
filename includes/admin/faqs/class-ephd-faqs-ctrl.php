...
class EPHD_FAQs_Ctrl {
	...
	public function get_question_data() {
		...
		foreach ( $question_all_lang as $question ) {
			$questions_esc[$question->lang] = [
				'title'    => sanitize_text_field( $question->question ),
		...
		foreach ( $widgets_config as $widget_config ) {
			...
			$faqs_preview_config = array(
				...
				'html'          => EPHD_HTML_Admin::get_item_preview_box( $widget_config, array(
					...
					'bottom_items_title'    => esc_html__( 'Used on', 'help-dialog' ) ) ) );
	...
	public function save_faqs() {
		...
		if ( empty( $faqs_name ) ) {
			EPHD_Utilities::ajax_show_error_die( esc_html__( 'FAQs topic cannot be empty.', 'help-dialog' ) );
		} */
		...
		if ( ! is_array( $faqs_sequence ) || empty( $faqs_sequence ) ) {
			EPHD_Utilities::ajax_show_error_die( esc_html__( 'FAQs should contain at least one question.', 'help-dialog' ) );
		}
	...
	public function load_faqs_form() {
		...
		// or use default to create a new one
			...
			if ( empty( $faqs_name ) ) {
				EPHD_Utilities::ajax_show_error_die( esc_html__( 'FAQs name cannot be empty.', 'help-dialog' ) );
			} */
