...
class EPHD_FAQs_Page {
		...
		foreach ( $this->widgets_config as $widget ) {
			...
			$faqs_boxes[] = array(
				...
				'html'  => EPHD_HTML_Admin::get_item_preview_box( $widget, array(
					...
					'sub_items_title'       => esc_html__( 'Questions', 'help-dialog' ),
					'icon_html'             => self::get_faqs_icon_html()
					//'bottom_items_title'    => esc_html__( 'Shown on these pages', 'help-dialog' )
			 ) ) );
		}
	...
	private function get_add_new_faq_box_html( $args=array() ) {
        ...
        <div class="ephd-admin__item-preview__footer">
            ...
            <div class="ephd-admin__item-preview__actions">   <?php
				...
				EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Create New FAQs', 'help-dialog' ), 'ephd_faqs', '', '', false, false, 'ephd-success-btn ephd-fp__create-new-faqs-btn' );  ?>
            </div>
        </div>  <?php
	...
	public function get_faqs_form_box_html( $widget ) {
		...
		<div class="ephd-fp__faqs-form__content">
            ...
            <div class="ephd-fp__faqs-form__row ephd-fp__questions-wrap">   <?php
                // All Questions
	            $widget_faqs_escaped = EPHD_Widgets_Page::get_tab_content_widget_faqs( $widget, $this->all_faqs );
	            echo $widget_faqs_escaped; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	...
	private function get_regular_view_config() {
		...
		$views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'FAQs', 'help-dialog' ),
			'icon_class' => 'ephdfa ep_font_icon_faq_icons',

			// Boxes List
			'list_top_actions_html' => EPHD_HTML_Admin::get_welcome_message( 'img/faqs-form-preview.jpg', esc_html__( 'Configure Help Dialog FAQs', 'help-dialog' ),
						esc_html__( 'Create Questions and Answers and group them in FAQ groups.', 'help-dialog' ) ), // An FAQ group can be show in any Help Dialog widget.', 'help-dialog' ) )
		...
		/*$views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Articles', 'help-dialog' ),
	...
	private static function get_faqs_form_actions_html() {
		...
		ob_start();  ?>
			...
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Save', 'help-dialog' ), 'ephd_save_faqs', 'ephd__save_faqs_wrap', '', false, '', 'ephd-success-btn' );
