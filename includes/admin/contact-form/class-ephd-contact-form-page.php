...
class EPHD_Contact_Form_Page {
		...
		<div id="ephd-admin-page-wrap">
			...
			<div class="ephd-contact-form-page-container">				<?php
				...
				EPHD_HTML_Forms::dialog_confirm_action( array(
					...
					'title'             => esc_html__( 'Deleting Design', 'help-dialog' ),
					'body'              => esc_html__( 'Are you sure you want to delete the Design? You cannot undo this action.', 'help-dialog' ),
	...
	private function get_contact_form_editor_boxes() {
		...
		$contact_form_boxes[] = array(
			'title' => esc_html__( 'Email Settings', 'help-dialog' ),
		...
		$contact_form_boxes[] = array(
			'title' => esc_html__( 'Email Delivery Test', 'help-dialog' ),
        ...
        <div class="ephd-cf__email-form__content">
            <ul class="ephd-cf__email-fields">   <?php
	            EPHD_HTML_Elements::text( [
		            ...
		            'tooltip_body' => esc_html__( 'Enter an email address in order to receive email notifications when a user submits a message through the Contact form.', 'help-dialog' ),
	            ] );    ?>
            </ul>
        </div>  <?php
	...
	private function get_test_contact_form_submission_box_html( $contact_submission_email ) {
        ...
        <div class="ephd-cf__test-email-form">
            ...
            <div class="ephd-cf__test-email-form__actions">
                ...
                EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Test Submission', 'help-dialog' ), 'ephd_test_contact_form_submission', '', '', false, '', 'ephd-primary-btn' );    ?>
            </div>
        </div>  <?php
	...
	private static function settings_tab_actions_row() {
        ...
        <div class="ephd-admin__list-actions-row"><?php
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Save Settings', 'help-dialog' ), 'ephd_save_contact_form_settings', '', '', false, '', 'ephd-success-btn');    ?>
        </div>      <?php
	...
	private function get_regular_views_config() {
		...
		$views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Email Notification', 'help-dialog' ),
		...
		$views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Submissions', 'help-dialog' ),
			...
			'boxes_list' => array(
				...
				array(
					...
					'title' => esc_html__( 'Contact Us Entries', 'help-dialog' ),
		...
		/*$views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Archived Submissions', 'help-dialog' ),
		...
		/*$views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Notification Rules', 'help-dialog' ),
		...
		/* $views_config[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Contact Form Design', 'help-dialog' ),
			'icon_class' => 'ephdfa ephdfa-paint-brush',

			// Boxes List
			'list_top_actions_html' => EPHD_HTML_Admin::get_welcome_message( 'img/contact-form-preview-2.jpg', esc_html__( 'Configure Contact Form', 'help-dialog' ),
							esc_html__( 'Change the contact form title and input labels.', 'help-dialog' ) ),
	...
	private static function get_submissions_actions() {
		...
		<div class="ephd-admin__list-actions-row">    <?php
			...
			EPHD_HTML_Forms::dialog_confirm_action( array(
				...
				'title'             => esc_html__( 'Deleting Submissions', 'help-dialog' ),
				'body'              => esc_html__( 'Are you sure you want to delete all submissions? You cannot undo this action.', 'help-dialog' ),
				'accept_label'      => esc_html__( 'Delete', 'help-dialog' ),
				'accept_type'       => 'warning',
	...
	private static function get_designs_bottom_actions_row() {
		...
		<div class="ephd-admin__list-actions-row">  <?php
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Create a New Design for the Contact Form', 'help-dialog' ), 'ephd_contact_form', '', '', false, false, 'ephd-success-btn ephd-cf__create-new-design-btn' ); ?>
		</div>  <?php
	...
	public function get_design_form_box_html( $contact_form ) {
		...
		<div class="ephd-cf__design-form__content">
			...
			EPHD_HTML_Forms::notification_box_middle( array(
				...
				'desc' => esc_html__( 'The contact form is designed with a built-in protection of submissions by bots.', 'help-dialog' ),
			) );
			...
			if ( $contact_form['contact_form_id'] > EPHD_Config_Specs::DEFAULT_ID ) {
				EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Delete', 'help-dialog' ), 'ephd_delete_contact_form', 'ephd-cf__delete-contact-form-wrap', '', false, '', 'ephd-error-btn' );
			}   ?>
	...
	public function get_config_of_contact_form_preview_box( $contact_form, $return_html=false, $active=true, $limited_preview=true ) {
		...
		$widgets_list_title = esc_html__( 'Used on these Widgets', 'help-dialog' ) . ':';
		$no_widgets_text = esc_html__( 'Not assigned to any Widget', 'help-dialog' );
		...
		if ( count( $this->contact_forms_config ) == 1 && count( $widgets_list ) == count( $this->widgets_config ) ) {
			$widgets_list = [];
			$widgets_list_title = esc_html__( 'Used on all widgets', 'help-dialog' );
			$no_widgets_text = '';
		}
	...
	private function get_widgets_list( $contact_form ) {
		...
		foreach ( $this->widgets_config as $widget ) {
			...
			$widget_in_list->post_title = $widget['widget_name'] . ' ' . esc_html__( 'Widget', 'help-dialog' );
	...
	private static function get_contact_form_actions_html( $contact_form ) {
		...
		if ( $contact_form['contact_form_id'] > 0 ) {   ?>
			...
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Save', 'help-dialog' ), 'ephd_save_contact_form', 'ephd__save_contact_form_wrap', '', false, '', 'ephd-success-btn' );

			// Create New Design
		} else {
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Create', 'help-dialog' ), 'ephd_save_contact_form', 'ephd__save_contact_form_wrap', '', false, '', 'ephd-success-btn' );
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Cancel', 'help-dialog' ), 'ephd_cancel_contact_form', 'ephd-cf__cancel_contact_form_wrap', '', false, '', 'ephd-error-btn' );
		}
